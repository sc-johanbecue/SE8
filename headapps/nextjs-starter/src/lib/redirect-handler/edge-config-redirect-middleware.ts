...
export class EdgeConfigRedirectsMiddleware extends RedirectsMiddleware {
  ...
  private edgeConfigHandler = async (
    ...
    const createResponse = async () => {
      ...
      const url = this.normalize(req.nextUrl.clone());
      ...
      if (REGEXP_ABSOLUTE_URL.test(existsRedirect.target)) {
        ...
        const target = existsRedirect.target;
      ...
      switch (existsRedirect.redirectType) {
        case REDIRECT_TYPE_301: {
          return this.createTheRedirectResponse(redirectUrl, res, 301, 'Moved Permanently');
        }
        case REDIRECT_TYPE_302: {
          return this.createTheRedirectResponse(redirectUrl, res, 302, 'Found');
        }
      ...
      ? modifyRedirects.find((redirect: RedirectInfo) => {
          ...
          return (
            (regexParser(redirect.pattern).test(tragetURL) ||
              ...
              pureRedirectPattern === targetHREF) &&
            (redirect.locale
   ...
   */
  private createTheRedirectResponse(
    url: string,
